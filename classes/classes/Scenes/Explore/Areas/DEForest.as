...
import classes.Scenes.Camp.CabinProgress;
    ...
    public class DEForest extends BaseContent {
        ...
        private var exploreAvailable:Boolean = true;
        private var woodAvailable:Boolean = false;

        public function DEForest() {
             
        }

        public function DEForestMenu() {
            outputText("You reach inside of your belongings and retrieve a map that you've drawn out of the local forest. You take a second to ponder whether to investigate one of the areas further, or to try and discover somehting new. \n\n");
        ...
        protected function foight() {
            kGAMECLASS.decombat.StartCombat(kGAMECLASS.demonlib.Goblin);
        }

        protected function searchForWood() {
            clearOutput();

            outputText("Deciding you need to get some wood, you reach down to your crotch, only to realise there's something wrong with your logic. Realising your error, you set out to the glades.\n\n");
            if(rand(100) > flags[kFLAGS.CORRUPTED_GLADES_DESTROYED]) {

            }

        }

        protected function exploreForest() {
            clearOutput();
