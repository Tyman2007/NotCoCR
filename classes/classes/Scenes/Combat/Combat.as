	...
	public class Combat extends BaseContent
	{
		...
		public function attack():void {
			...
			var getBase:Function = function(init:Number):Number {
				if (player.findPerk(PerkLib.DoubleAttack) >= 0 && player.spe >= 50 && init > 76 + (player.newGamePlusMod() * 15) && flags[kFLAGS.DOUBLE_ATTACK_STYLE] == 0) {
					return 75.5 + (player.newGamePlusMod() * 15);
				} else return init;
			};
			
			// init value depending on weapon type
			if (isWieldingRangedWeapon()) {
				if (player.weaponName.indexOf("staff") != -1) damage = Math.pow(getBase.call(null, player.inte),2)*0.1;
				else damage = Math.pow(getBase.call(null, player.spe),2)*0.05; // woudn't be better to use speed as base and int as extra?
			}
			else
				damage = Math.pow(getBase.call(null, player.str),2)*0.05;
